  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (data._id && data._id !== '') {
        ...
        const existingInvoice = await tenantInvoice
          .findById(data._id)
          .lean()
          .exec()
        previousInvoiceDate = existingInvoice
      ...
      // If the invoice's date changed, the old month must also be refreshed -
      ...
      try {
        ...
        if (
          previousInvoiceDate &&
          (previousInvoiceDate.getFullYear() !== year ||
            previousInvoiceDate.getMonth() + 1 !== month)
        ) {
