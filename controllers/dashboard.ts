...
import { PERIOD_PRESETS } from '../common/constants/periodPresets'
  ...
  try {
    ...
    if (
      ...
      if (preCalculatedStats && preCalculatedStats.length > 0) {
        ...
        // No pre-calculated stats available — fall back to dynamic aggregation for
        // chart data but still return real-time summaryMetrics so the source stays
        // consistent regardless of whether DashboardStats docs exist.
        const yearStart = new Date(`${targetYear}-01-01T00:00:00.000Z`)
        const yearEnd = new Date(`${targetYear}-12-31T23:59:59.999Z`)
        const dynamicData = await dynamicAggregation(
          tenantId,
          yearStart,
          yearEnd,
          getGroupingLevel(
        ...
        return res.json({
          success: true,
          data: {
            labels: dynamicData.map((item) => item.period),
            turnover: dynamicData.map((item) => item.totalRevenue || 0),
            expenses: dynamicData.map((item) => item.totalExpenses || 0),
            summaryMetrics: {
              totalRevenue: yearlyMetrics.totalRevenue,
              totalExpenses: yearlyMetrics.totalExpenses,
              netProfit: yearlyMetrics.netProfit,
